...
public class MenuPage : MonoBehaviour
{
    ...
    [ShowIf("selectFirstSelectableOnEnable")][SerializeField] private float selectFirstSelectableDelay = 0;[EndIf]
    ...
    [EndIf]
    [Header("DeSelected")]
    [SerializeField] private bool moveOutAnimation = false;
    [FormerlySerializedAs("moveOutFromDirection")]
    [ShowIf("moveOutAnimation")]
    ...
    private void Awake()
    {
        ...
        if (!MenuCategory || !MenuController)
        {
    ...
    public virtual void OnPageSelected()
    {
        if (moveInAnimation) 
        {
            ...
            canSelect = true;
            ResetAllSelectables();
            StartCoroutine(SelectFirstAvailableSelectableCar(selectFirstSelectableDelay)); 
        }
    }
    ...
    public void OnNavigate(Vector2 input) // Input event
    {
        if (!canSelect) return;
...
#region Selectables Events // ---------------------------------------------------------------------
    ...
    private void OnSelect(BaseEventData eventData)
    {
        ...
        sfxSelectableSelected?.Play(MenuController.AudioSource);
    }
...
#region Page Management // ---------------------------------------------------------------------
    ...
    protected void SetupSelectable(Selectable selectable)
    {
        ...
        {
            button.onClick.AddListener(() => sfxButtonClick?.Play(MenuController.AudioSource));
        }
    }
    

    protected void AddEventTriggerEntry(EventTrigger eventTrigger, EventTriggerType type, UnityEngine.Events.UnityAction<BaseEventData> callback)
    {
    ...
    [Button] protected void ResetAllSelectables() 
    {
    ...
    protected IEnumerator SelectFirstAvailableSelectableCar(float delay = 0)
    {
    ...
    protected void ResetLayoutGroups()
    {
    ...
    protected void DisableAllLayoutGroups()
    {
        if (_layoutGroups.Count == 0) return;
...
#region Animations // ---------------------------------------------------------------------
    ...
    [Button] private void PlayMoveInAnimation()
    { 
        
        #if UNITY_EDITOR // Only play the animation when in play mode
        if (Application.isPlaying == false) return;
        #endif
        
        if (moveInObjects.Count == 0) // If there are no objects to animate start page normally
        {
            canSelect = true;
            ResetAllSelectables();
        ...
        StartCoroutine(SelectFirstAvailableSelectableCar(totalAnimationTime)); 
    }
    ...
    [Button] private void PlayMoveOutAnimation()
    {
        #if UNITY_EDITOR // Only play the animation when in play mode
        if (Application.isPlaying == false) return;
        #endif
        
        if (moveOutObjects.Count == 0) // If there are no objects to animate start page normally
        {
            return;
        }
        ...
        float totalAnimationTime = moveOutDuration + (moveOutDelay * (moveOutObjects.Count - 1));
    }
